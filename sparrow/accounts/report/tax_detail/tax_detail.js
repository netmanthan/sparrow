...
sparrow.TaxDetail = class TaxDetail {
	constructor() {
		this.patch();
		this.load_report();
	}
	...
	refresh_report() {
		...
		if (this.qr.report_name !== 'Tax Detail') {
			frappe.call({
				method: 'sparrow.accounts.report.tax_detail.tax_detail.get_custom_reports',
	...
	save_report() {
		...
		if (this.qr.report_name !== 'Tax Detail') {
			frappe.call({
				method:'sparrow.accounts.report.tax_detail.tax_detail.save_custom_report',
				args: {
...
if (!window.taxdetail) {
	window.taxdetail = new sparrow.TaxDetail();
}
