...
{% include 'sparrow/selling/sales_common.js' %};
frappe.provide("sparrow.accounts");


sparrow.accounts.SalesInvoiceController = class SalesInvoiceController extends sparrow.selling.SellingController {
	setup(doc) {
		this.setup_posting_date_time_check();
		super.setup(doc);
	}
	company() {
		sparrow.accounts.dimensions.update_dimension(this.frm, this.frm.doctype);

		let me = this;
		if (this.frm.doc.company) {
			frappe.call({
				method:
					"sparrow.accounts.party.get_party_account",
				args: {
					party_type: 'Customer',
	...
	onload() {
		...
		sparrow.queries.setup_queries(this.frm, "Warehouse", function() {
			return sparrow.queries.warehouse(me.frm.doc);
		});
		...
		sparrow.queries.setup_warehouse_query(this.frm);
	}
	...
	refresh(doc, dt, dn) {
		...
		if(doc.docstatus==1 && !doc.is_return) {
			...
			if(!doc.auto_repeat) {
				cur_frm.add_custom_button(__('Subscription'), function() {
					sparrow.utils.make_subscription(doc.doctype, doc.name)
				}, __('Create'))
			}
		}
		...
		sparrow.accounts.unreconcile_payments.add_unreconcile_btn(me.frm);
	}


	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "sparrow.accounts.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: cur_frm
		})
	}
		...
		this.$sales_order_btn = this.frm.add_custom_button(__('Sales Order'),
			function() {
				sparrow.utils.map_current_doc({
					method: "sparrow.selling.doctype.sales_order.sales_order.make_sales_invoice",
	...
	quotation_btn() {
		...
		this.$quotation_btn = this.frm.add_custom_button(__('Quotation'),
			function() {
				sparrow.utils.map_current_doc({
					method: "sparrow.selling.doctype.quotation.quotation.make_sales_invoice",
	...
	delivery_note_btn() {
		...
		this.$delivery_note_btn = this.frm.add_custom_button(__('Delivery Note'),
			function() {
				sparrow.utils.map_current_doc({
					method: "sparrow.stock.doctype.delivery_note.delivery_note.make_sales_invoice",
					...
					get_query: function() {
						...
						return {
							query: "sparrow.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters
						};
					}
				});
	...
	customer() {
		...
		sparrow.utils.get_party_details(this.frm,
			"sparrow.accounts.party.get_party_details", {
		...
		if(this.frm.doc.customer) {
			frappe.call({
				"method": "sparrow.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				"args": {
	...
	make_inter_company_invoice() {
		let me = this;
		frappe.model.open_mapped_doc({
			method: "sparrow.accounts.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm
		});
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "sparrow.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: cur_frm
		})
	}
	...
	asset(frm, cdt, cdn) {
		...
		if(row.asset) {
			frappe.call({
				method: sparrow.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
...
extend_cscript(cur_frm.cscript, new sparrow.accounts.SalesInvoiceController({frm: cur_frm}));

cur_frm.cscript['Make Delivery Note'] = function() {
	frappe.model.open_mapped_doc({
		method: "sparrow.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm
	})
}
...
cur_frm.set_query("income_account", "items", function(doc) {
	return{
		query: "sparrow.controllers.queries.get_income_account",
...
cur_frm.cscript.income_account = function(doc, cdt, cdn) {
	sparrow.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
}

cur_frm.cscript.expense_account = function(doc, cdt, cdn) {
	sparrow.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
}

cur_frm.cscript.cost_center = function(doc, cdt, cdn) {
	sparrow.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
}
...
frappe.ui.form.on('Sales Invoice', {
	setup: function(frm){
		...
		frm.fields_dict["timesheets"].grid.get_field("time_sheet").get_query = function(doc, cdt, cdn){
			return{
				query: "sparrow.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.set_query('pos_profile', function(doc) {
			...
			return {
				query: 'sparrow.accounts.doctype.pos_profile.pos_profile.pos_profile_query',
				filters: {
					company: doc.company
				}
			};
	...
	company: function(frm){
		if (frm.doc.company) {
			frappe.call({
				method: "sparrow.setup.doctype.company.company.get_default_company_address",
	...
	get_loyalty_details: function(frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			frappe.call({
				method: "sparrow.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
					"customer": frm.doc.customer,
					"loyalty_program": frm.doc.loyalty_program,
	...
	async get_timesheet_data(frm, kwargs) {
		return frappe.call({
			method: "sparrow.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
			args: kwargs
		}).then(r => {
	...
	create_invoice_discounting: function(frm) {
		frappe.model.open_mapped_doc({
			method: "sparrow.accounts.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: frm
		});
	},
	...
	create_dunning: function(frm) {
		frappe.model.open_mapped_doc({
			method: "sparrow.accounts.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: frm
		});
	}
});
...
var set_timesheet_detail_rate = function(cdt, cdn, currency, timelog) {
	frappe.call({
		method: "sparrow.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency
		},
