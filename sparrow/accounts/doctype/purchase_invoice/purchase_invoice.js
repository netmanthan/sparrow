...
sparrow.accounts.PurchaseInvoice = class PurchaseInvoice extends sparrow.buying.BuyingController {
	setup(doc) {
	...
	unblock_invoice() {
		const me = this;
		frappe.call({
			'method': 'sparrow.accounts.doctype.purchase_invoice.purchase_invoice.unblock_invoice',
	...
	make_comment_dialog_and_block_invoice(){
		...
		this.dialog.set_primary_action(__('Save'), function() {
			...
			frappe.call({
				'method': 'sparrow.accounts.doctype.purchase_invoice.purchase_invoice.block_invoice',
				'args': {
	...
	set_release_date(data) {
		return frappe.call({
			'method': 'sparrow.accounts.doctype.purchase_invoice.purchase_invoice.change_release_date',
			'args': data,
	...
	supplier() {
		...
		sparrow.utils.get_party_details(this.frm, "sparrow.accounts.party.get_party_details",
			{
	...
	make_inter_company_invoice(frm) {
		frappe.model.open_mapped_doc({
			method: "sparrow.accounts.doctype.purchase_invoice.purchase_invoice.make_inter_company_sales_invoice",
			frm: frm
		});
	}
	...
	make_debit_note() {
		frappe.model.open_mapped_doc({
			method: "sparrow.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note",
			frm: cur_frm
		})
	}
};

cur_frm.script_manager.make(sparrow.accounts.PurchaseInvoice);
