...
frappe.ui.form.on('Payment Order', {
	...
	get_from_payment_entry: function(frm) {
		...
		sparrow.utils.map_current_doc({
			method: "sparrow.accounts.doctype.payment_entry.payment_entry.make_payment_order",
	...
	get_from_payment_request: function(frm) {
		frm.trigger("remove_row_if_empty");
		sparrow.utils.map_current_doc({
			method: "sparrow.accounts.doctype.payment_request.payment_request.make_payment_order",
	...
	make_payment_records: function(frm){
		var dialog = new frappe.ui.Dialog({
			...
			fields: [
				{"fieldtype": "Link", "label": __("Supplier"), "fieldname": "supplier", "options":"Supplier",
					"get_query": function () {
						return {
							query:"sparrow.accounts.doctype.payment_order.payment_order.get_supplier_query",
				...
				{"fieldtype": "Link", "label": __("Mode of Payment"), "fieldname": "mode_of_payment", "options":"Mode of Payment",
					"get_query": function () {
						return {
							query:"sparrow.accounts.doctype.payment_order.payment_order.get_mop_query",
							filters: {'parent': frm.doc.name}
						}
					}
				}
		...
		dialog.set_primary_action(__("Submit"), function() {
			...
			return frappe.call({
				method: "sparrow.accounts.doctype.payment_order.payment_order.make_payment_records",
				args: {
