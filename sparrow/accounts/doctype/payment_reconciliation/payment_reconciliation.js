...
sparrow.accounts.PaymentReconciliationController = class PaymentReconciliationController extends frappe.ui.form.Controller {
	onload() {
		...
		if (this.frm.doc.receivable_payable_account) {
			this.frm.call({
				...
				callback: (r) => {
					if (r.message) {
						this.frm.call({
							'method': "sparrow.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.is_any_doc_running",
							"args": {
								for_filter: {
									company: this.frm.doc.company,
...
extend_cscript(cur_frm.cscript, new sparrow.accounts.PaymentReconciliationController({frm: cur_frm}));
