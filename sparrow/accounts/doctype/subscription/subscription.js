...
frappe.ui.form.on('Subscription', {
	...
	cancel_this_subscription: function(frm) {
		...
		frappe.confirm(
			...
			function() {
				frappe.call({
					method:
					"sparrow.accounts.doctype.subscription.subscription.cancel_subscription",
					args: {name: doc.name},
	...
	renew_this_subscription: function(frm) {
		...
		frappe.confirm(
			...
			function() {
				frappe.call({
					method:
					"sparrow.accounts.doctype.subscription.subscription.restart_subscription",
					args: {name: doc.name},
					callback: function(data){
						if(!data.exc){
							frm.reload_doc();
	...
	get_subscription_updates: function(frm) {
		...
		frappe.call({
			method:
			"sparrow.accounts.doctype.subscription.subscription.get_subscription_updates",
			args: {name: doc.name},
			freeze: true,
			callback: function(data){
				if(!data.exc){
