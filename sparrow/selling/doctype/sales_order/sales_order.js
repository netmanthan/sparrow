...
frappe.ui.form.on("Sales Order", {
	...
	onload: function(frm) {
		...
		frm.set_query('project', function(doc, cdt, cdn) {
			return {
				query: "sparrow.controllers.queries.get_project_name",
				filters: {
		...
		frm.set_query('warehouse', 'items', function(doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "sparrow.controllers.queries.warehouse_query";
...
frappe.ui.form.on("Sales Order Item", {
	...
	delivery_date: function(frm, cdt, cdn) {
		if(!frm.doc.delivery_date) {
			sparrow.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	}
});

sparrow.selling.SalesOrderController = class SalesOrderController extends sparrow.selling.SellingController {
	onload(doc, dt, dn) {
	...
	refresh(doc, dt, dn) {
		...
		if (this.frm.doc.docstatus===0) {
			this.frm.add_custom_button(__('Quotation'),
				function() {
					let d = sparrow.utils.map_current_doc({
						method: "sparrow.selling.doctype.quotation.quotation.make_sales_order",
	...
	create_pick_list() {
		frappe.model.open_mapped_doc({
			method: "sparrow.selling.doctype.sales_order.sales_order.create_pick_list",
			frm: this.frm
		})
	}
	...
	make_work_order() {
		var me = this;
		me.frm.call({
			method: "sparrow.selling.doctype.sales_order.sales_order.get_work_order_items",
			args: {
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new frappe.ui.Dialog({
			...
			primary_action: function() {
				...
				me.frm.call({
					method: 'sparrow.selling.doctype.sales_order.sales_order.make_raw_material_request',
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
	...
	make_delivery_note(delivery_dates) {
		frappe.model.open_mapped_doc({
			method: "sparrow.selling.doctype.sales_order.sales_order.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates
			}
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "sparrow.selling.doctype.sales_order.sales_order.make_maintenance_schedule",
			frm: this.frm
		})
	}
	...
	make_project() {
		frappe.model.open_mapped_doc({
			method: "sparrow.selling.doctype.sales_order.sales_order.make_project",
			frm: this.frm
		})
	}
	...
	make_inter_company_order() {
		frappe.model.open_mapped_doc({
			method: "sparrow.selling.doctype.sales_order.sales_order.make_inter_company_purchase_order",
			frm: this.frm
		});
	}
	...
	make_maintenance_visit() {
		frappe.model.open_mapped_doc({
			method: "sparrow.selling.doctype.sales_order.sales_order.make_maintenance_visit",
			frm: this.frm
		})
	}
	...
	make_purchase_order(){
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action (args) {
				...
				return frappe.call({
					method: "sparrow.selling.doctype.sales_order.sales_order." + method,
					freeze: true,
	...
	update_status(label, status){
		...
		frappe.call({
			method: "sparrow.selling.doctype.sales_order.sales_order.update_status",
...
extend_cscript(cur_frm.cscript, new sparrow.selling.SalesOrderController({frm: cur_frm}));
