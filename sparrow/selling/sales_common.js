...
{% include 'sparrow/accounts/doctype/sales_taxes_and_charges_template/sales_taxes_and_charges_template.js' %}


cur_frm.email_field = "contact_email";

frappe.provide("sparrow.selling");
sparrow.selling.SellingController = class SellingController extends sparrow.TransactionController {
	setup() {
		super.setup();
	}
		...
		$.each([["customer", "customer"],
			...
			function(i, opts) {
				if(me.frm.fields_dict[opts[0]])
					me.frm.set_query(opts[0], sparrow.queries[opts[1]]);
			});

		me.frm.set_query('contact_person', sparrow.queries.contact_query);
		me.frm.set_query('customer_address', sparrow.queries.address_query);
		me.frm.set_query('shipping_address_name', sparrow.queries.address_query);
		me.frm.set_query('dispatch_address_name', sparrow.queries.dispatch_address_query);
	...
	customer_address() {
		sparrow.utils.get_address_display(this.frm, "customer_address");
		sparrow.utils.set_taxes_from_address(this.frm, "customer_address", "customer_address", "shipping_address_name");
	}

	shipping_address_name() {
		sparrow.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
		sparrow.utils.set_taxes_from_address(this.frm, "shipping_address_name", "customer_address", "shipping_address_name");
	}

	dispatch_address_name() {
		sparrow.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
	}
		...
		frappe.db.get_value('Item', {'item_code': item.item_code}, 'has_batch_no', (r) => {
			...
			if(item.item_code && item.warehouse) {
				return this.frm.call({
					method: "sparrow.stock.get_item_details.get_bin_details_and_serial_nos",
					child: item,
					args: {
	...
	batch_no(doc, cdt, cdn) {
		...
		frappe.db.get_value('Item', {'item_code': item.item_code}, 'has_serial_no', (r) => {
			...
			if(item.warehouse && item.item_code && item.batch_no) {
				return this.frm.call({
					method: "sparrow.stock.get_item_details.get_batch_qty_and_serial_no",
					child: item,
					args: {
	...
	*/
	...
	_set_batch_number(doc) {
		...
		return frappe.call({
			method: 'sparrow.stock.doctype.batch.batch.get_batch_no',
			args: args,
			callback: function(r) {
				if(r.message) {
...
frappe.ui.form.on(cur_frm.doctype,"project", function(frm) {
	if(in_list(["Delivery Note", "Sales Invoice"], frm.doc.doctype)) {
		if(frm.doc.project) {
			frappe.call({
				method:'sparrow.projects.doctype.project.project.get_cost_center_name' ,
