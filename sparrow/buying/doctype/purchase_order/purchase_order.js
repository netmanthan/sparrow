...
frappe.provide("sparrow.buying");
frappe.provide("sparrow.accounts.dimensions");
{% include 'sparrow/public/js/controllers/buying.js' %};
...
frappe.ui.form.on("Purchase Order", {
	setup: function(frm) {
		...
		frm.set_query("expense_account", "items", function() {
			return {
				query: "sparrow.controllers.queries.get_expense_account",
	...
	get_materials_from_supplier: function(frm) {
		...
		if (po_details && po_details.length) {
			frm.add_custom_button(__('Return of Components'), () => {
				frm.call({
					method: 'sparrow.controllers.subcontracting_controller.get_materials_from_supplier',
					freeze: true,
	...
	onload: function(frm) {
		...
		sparrow.queries.setup_queries(frm, "Warehouse", function() {
			return sparrow.queries.warehouse(frm.doc);
		});
	...
	schedule_date: function(frm, cdt, cdn) {
		...
		if (row.schedule_date) {
			if(!frm.doc.schedule_date) {
				sparrow.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
sparrow.buying.PurchaseOrderController = class PurchaseOrderController extends sparrow.buying.BuyingController {
	setup() {
				...
				if(this.frm.doc.status !== 'Closed' && flt(this.frm.doc.per_received) < 100 && flt(this.frm.doc.per_billed) < 100) {
					...
					if (!(this.frm.doc.is_subcontracted && !this.frm.doc.is_old_subcontracting_flow)) {
						this.frm.add_custom_button(__('Update Items'), () => {
							sparrow.utils.update_child_items({
								frm: this.frm,
								child_docname: "items",
	...
	get_items_from_open_material_requests() {
		sparrow.utils.map_current_doc({
			method: "sparrow.stock.doctype.material_request.material_request.make_purchase_order_based_on_supplier",
			args: {
			...
			get_query_method: "sparrow.stock.doctype.material_request.material_request.get_material_requests_based_on_supplier"
		});
	}
	...
	make_stock_entry() {
		frappe.call({
			method:"sparrow.controllers.subcontracting_controller.make_rm_stock_entry",
			args: {
	...
	make_inter_company_order(frm) {
		frappe.model.open_mapped_doc({
			method: "sparrow.buying.doctype.purchase_order.purchase_order.make_inter_company_sales_order",
			frm: frm
		});
	}
	...
	make_purchase_receipt() {
		frappe.model.open_mapped_doc({
			method: "sparrow.buying.doctype.purchase_order.purchase_order.make_purchase_receipt",
			frm: cur_frm,
	...
	make_purchase_invoice() {
		frappe.model.open_mapped_doc({
			method: "sparrow.buying.doctype.purchase_order.purchase_order.make_purchase_invoice",
			frm: cur_frm
		})
	}
	...
	make_subcontracting_order() {
		frappe.model.open_mapped_doc({
			method: "sparrow.buying.doctype.purchase_order.purchase_order.make_subcontracting_order",
			frm: cur_frm,
	...
	add_from_mappers() {
		...
		this.frm.add_custom_button(__('Material Request'),
			function() {
				sparrow.utils.map_current_doc({
					method: "sparrow.stock.doctype.material_request.material_request.make_purchase_order",
					source_doctype: "Material Request",
					target: me.frm,
					setters: {
		...
		this.frm.add_custom_button(__('Supplier Quotation'),
			function() {
				sparrow.utils.map_current_doc({
					method: "sparrow.buying.doctype.supplier_quotation.supplier_quotation.make_purchase_order",
		...
		function() {
			...
			frappe.call({
				method: "sparrow.buying.utils.get_linked_material_requests",
				args:{
					items: my_items
				},
				callback: function(r) {
...
extend_cscript(cur_frm.cscript, new sparrow.buying.PurchaseOrderController({frm: cur_frm}));

cur_frm.cscript.update_status= function(label, status){
	frappe.call({
		method: "sparrow.buying.doctype.purchase_order.purchase_order.update_status",
...
function set_schedule_date(frm) {
	if(frm.doc.schedule_date){
		sparrow.utils.copy_value_in_all_rows(frm.doc, frm.doc.doctype, frm.doc.name, "items", "schedule_date");
	}
}
...
frappe.ui.form.on("Purchase Order", "is_subcontracted", function(frm) {
	if (frm.doc.is_old_subcontracting_flow) {
		sparrow.buying.get_default_bom(frm);
	}
});