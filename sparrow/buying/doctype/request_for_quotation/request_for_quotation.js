	...
	refresh: function(frm, cdt, cdn) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(__("Send Emails to Suppliers"), function() {
				frappe.call({
					method: 'sparrow.buying.doctype.request_for_quotation.request_for_quotation.send_supplier_emails',
					freeze: true,
					args: {
						...
						(data) => {
							var w = window.open(
								frappe.urllib.get_full_url(
									"/api/method/sparrow.buying.doctype.request_for_quotation.request_for_quotation.get_pdf?" +
									new URLSearchParams({
										name: frm.doc.name,
	...
	make_supplier_quotation: function(frm) {
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action: (args) => {
				...
				return frappe.call({
					type: "GET",
					method: "sparrow.buying.doctype.request_for_quotation.request_for_quotation.make_supplier_quotation_from_rfq",
					args: {
...
sparrow.buying.RequestforQuotationController = class RequestforQuotationController extends sparrow.buying.BuyingController {
	refresh() {
		var me = this;
		super.refresh();
		if (this.frm.doc.docstatus===0) {
			...
			this.frm.add_custom_button(__('Possible Supplier'), function() {
				...
				var dialog = new frappe.ui.Dialog({
					...
					primary_action: (args) => {
						...
						sparrow.utils.map_current_doc({
							method: "sparrow.buying.doctype.request_for_quotation.request_for_quotation.get_item_from_material_requests_based_on_supplier",
			...
			this.frm.add_custom_button(__('Link to Material Requests'),
				function() {
					sparrow.buying.link_to_mrs(me.frm);
				}, __("Tools"));
			...
			fields: [
				{
					...
					onchange() {
						if(dialog.get_value('search_type') == 'Tag'){
							frappe.call({
								method: 'sparrow.buying.doctype.request_for_quotation.request_for_quotation.get_supplier_tag',
							}).then(r => {
...
extend_cscript(cur_frm.cscript, new sparrow.buying.RequestforQuotationController({frm: cur_frm}));
