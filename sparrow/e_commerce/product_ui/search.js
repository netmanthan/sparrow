sparrow.ProductSearch = class {
	...
	bindSearchAction() {
		...
		this.searchBox.on("input", (e) => {
			...
			frappe.call({
				method: "sparrow.templates.pages.product_search.search",
				args: {
					query: query
				},
				callback: (data) => {
	...
	populateResults(product_results) {
		...
		product_results.forEach((res) => {
			let thumbnail = res.thumbnail || '/assets/sparrow/images/ui-states/cart-empty-state.png';
			html += `
