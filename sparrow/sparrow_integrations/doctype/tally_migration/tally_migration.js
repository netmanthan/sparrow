...
frappe.provide("sparrow.tally_migration");
...
frappe.ui.form.on("Tally Migration", {
	...
	refresh: function (frm) {
		...
		sparrow.tally_migration.failed_import_log = JSON.parse(frm.doc.failed_import_log);
		sparrow.tally_migration.fixed_errors_log = JSON.parse(frm.doc.fixed_errors_log);
	...
	render_html_table(frm, shown_logs, hidden_logs, field) {
		...
		let rows = sparrow.tally_migration.get_html_rows(shown_logs, field);
	...
	show_error_summary(frm) {
		let summary = sparrow.tally_migration.failed_import_log.reduce((summary, row) => {
			if (row.doc) {
	...
	show_errored_import_log(frm) {
		let import_log = sparrow.tally_migration.failed_import_log;
	...
	show_fixed_errors_log(frm) {
		let completed_log = sparrow.tally_migration.fixed_errors_log;
...
sparrow.tally_migration.getError = (traceback) => {
...
sparrow.tally_migration.cleanDoc = (obj) => {
	...
	$.each(temp, function(key, value){
		if (value === "" || value === null){
			...
			sparrow.tally_migration.cleanDoc(value);
		} else if ($.isArray(value)) {
			$.each(value, function (k,v) { sparrow.tally_migration.cleanDoc(v); });
		}
	});
	return temp;
}

sparrow.tally_migration.unresolve = (document) => {
	/* Mark document migration as unresolved ie. move to failed error log */
	let frm = cur_frm;
	let failed_log = sparrow.tally_migration.failed_import_log;
	let fixed_log = sparrow.tally_migration.fixed_errors_log;

	let modified_fixed_log = fixed_log.filter(row => {
		if (!frappe.utils.deep_equal(sparrow.tally_migration.cleanDoc(row.doc), document)) {
			return row
		}
	});
...
sparrow.tally_migration.create_new_doc = (document) => {
...
sparrow.tally_migration.get_html_rows = (logs, field) => {
	let index = 0;
	let rows = logs
		.map(({ doc, exc }) => {
			...
			let error_message = sparrow.tally_migration.getError(traceback);
			index++;
			...
			let show_doc = `
				...
				<div class="collapse margin-top" id="${id}-doc">
					<div class="well">
						<pre style="font-size: smaller;">${JSON.stringify(sparrow.tally_migration.cleanDoc(doc), null, 1)}</pre>
					</div>
				</div>`;

			let create_button = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='sparrow.tally_migration.create_new_doc(${JSON.stringify(doc)})'>
					${__("Create Document")}
				</button>`

			let mark_as_unresolved = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='sparrow.tally_migration.unresolve(${JSON.stringify(doc)})'>
