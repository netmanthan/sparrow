...
import sparrow
from sparrow.accounts.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
)
from sparrow.accounts.doctype.pricing_rule.utils import (
	apply_pricing_rule_for_free_items,
	apply_pricing_rule_on_transaction,
	get_applied_pricing_rules,
)
from sparrow.accounts.general_ledger import get_round_off_account_and_cost_center
from sparrow.accounts.party import (
	get_party_account,
	get_party_account_currency,
	get_party_gle_currency,
	validate_party_frozen_disabled,
)
from sparrow.accounts.utils import (
	create_gain_loss_journal,
	get_account_currency,
	get_fiscal_years,
	validate_fiscal_year,
)
from sparrow.buying.utils import update_last_purchase_rate
from sparrow.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
from sparrow.controllers.sales_and_purchase_return import validate_return
from sparrow.exceptions import InvalidCurrency
from sparrow.setup.utils import get_exchange_rate
from sparrow.stock.doctype.item.item import get_uom_conv_factor
from sparrow.stock.doctype.packed_item.packed_item import make_packing_list
from sparrow.stock.get_item_details import (
...
from sparrow.utilities.regional import temporary_flag
from sparrow.utilities.transaction_base import TransactionBase
...
class AccountsController(TransactionBase):
	...
	def company_currency(self):
		if not hasattr(self, "__company_currency"):
			self.__company_currency = sparrow.get_company_currency(self.company)
	...
	def validate_due_date(self):
		...
		from sparrow.accounts.party import validate_due_date
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if hasattr(item, "cost_center") and not item.get("cost_center"):
						item.set(
							"cost_center", self.get("cost_center") or sparrow.get_default_cost_center(self.company)
						)
	...
	def update_against_document_in_jv(self):
		...
		if lst:
			from sparrow.accounts.utils import reconcile_against_document
				...
				self.get_gl_dict(
					{
						...
						"cost_center": self.cost_center or sparrow.get_default_cost_center(self.company),
					},
					item=self,
				)
			)
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
		from sparrow.controllers.status_updater import get_allowance_for
	...
	def get_company_default(self, fieldname, ignore_validation=False):
		from sparrow.accounts.utils import get_company_default
	...
	def create_advance_and_reconcile(self, party_link):
		...
		reconcilation_entry.cost_center = self.cost_center or sparrow.get_default_cost_center(
			self.company
		)
		...
		advance_entry.cost_center = self.cost_center or sparrow.get_default_cost_center(self.company)
	...
	def check_conversion_rate(self):
		default_currency = sparrow.get_company_currency(self.company)
...
def update_bin_on_delete(row, doctype):
	...
	from sparrow.stock.stock_balance import (
		get_indented_qty,
		get_ordered_qty,
		get_reserved_qty,
		update_bin_qty,
