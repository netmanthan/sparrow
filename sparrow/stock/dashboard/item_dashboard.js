...
sparrow.stock.ItemDashboard = class ItemDashboard {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
		...
		function handle_move_add(element, action) {
			...
			if (disable_quick_entry) {
				...
				if (action === "Add") {
					...
					sparrow.stock.move_item(item, null, warehouse, actual_qty, rate, function () {
						me.refresh();
					});
				} else {
					sparrow.stock.move_item(item, warehouse, null, actual_qty, null, function () {
						me.refresh();
					});
				}
			}
...
sparrow.stock.move_item = function (item, source, target, actual_qty, rate, callback) {
