...
from sparrow.assets.doctype.asset.asset import get_asset_account, is_cwip_accounting_enabled
...
from sparrow.stock.doctype.delivery_note.delivery_note import make_inter_company_transaction
	...
	def make_item_gl_entries(self, gl_entries, warehouse_account=None):
		...
		for d in self.get("items"):
			if (
				...
				if not (
					(sparrow.is_perpetual_inventory_enabled(self.company) and d.item_code in stock_items)
					or d.is_fixed_asset
				):
					continue
...
def make_purchase_invoice(source_name, target_doc=None):
	from sparrow.accounts.party import get_payment_terms_template
