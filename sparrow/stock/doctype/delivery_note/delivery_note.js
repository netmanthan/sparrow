...
frappe.provide("sparrow.stock.delivery_note");
...
frappe.ui.form.on("Delivery Note", {
	setup: function(frm) {
		...
		sparrow.queries.setup_warehouse_query(frm);
		...
		frm.set_query('expense_account', 'items', function(doc, cdt, cdn) {
			if (sparrow.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
	...
	print_without_amount: function(frm) {
		sparrow.stock.delivery_note.set_print_hide(frm.doc);
	},
		...
		if (frm.doc.docstatus == 1 && !frm.doc.inter_company_reference) {
			...
			if (internal) {
				...
				frm.add_custom_button(__(button_label), function() {
					frappe.model.open_mapped_doc({
						method: 'sparrow.stock.doctype.delivery_note.delivery_note.make_inter_company_purchase_receipt',
						frm: frm,
					});
				}, __('Create'));
			}
...
sparrow.stock.DeliveryNoteController = class DeliveryNoteController extends sparrow.selling.SellingController {
	setup(doc) {
	...
	refresh(doc, dt, dn) {
			...
			if(doc.docstatus==0 && !doc.__islocal) {
				if (doc.__onload && doc.__onload.has_unpacked_items) {
					this.frm.add_custom_button(__('Packing Slip'), function() {
						frappe.model.open_mapped_doc({
							method: "sparrow.stock.doctype.delivery_note.delivery_note.make_packing_slip",
							frm: me.frm
						}) }, __('Create')
					);
				}
		...
		sparrow.stock.delivery_note.set_print_hide(doc, dt, dn);
	...
	make_shipment() {
		frappe.model.open_mapped_doc({
			method: "sparrow.stock.doctype.delivery_note.delivery_note.make_shipment",
			frm: this.frm
		})
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "sparrow.stock.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm
		})
	}
	...
	make_delivery_trip() {
		frappe.model.open_mapped_doc({
			method: "sparrow.stock.doctype.delivery_note.delivery_note.make_delivery_trip",
			frm: cur_frm
		})
	}
	...
	update_status(status) {
		...
		frappe.call({
			method:"sparrow.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
extend_cscript(cur_frm.cscript, new sparrow.stock.DeliveryNoteController({frm: cur_frm}));
...
frappe.ui.form.on('Delivery Note', {
	...
	unhide_account_head: function(frm) {
		...
		var aii_enabled = sparrow.is_perpetual_inventory_enabled(frm.doc.company)
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	}
})


sparrow.stock.delivery_note.set_print_hide = function(doc, cdt, cdn){
