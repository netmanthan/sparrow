...
frappe.provide("sparrow.stock");
frappe.provide("sparrow.accounts.dimensions");

{% include 'sparrow/stock/landed_taxes_and_charges_common.js' %};
...
frappe.ui.form.on('Stock Entry', {
	setup: function(frm) {
		...
		frm.set_query('batch_no', 'items', function(doc, cdt, cdn) {
			...
			if(!item.item_code) {
				...
				return {
					query : "sparrow.controllers.queries.get_batch_no",
					filters: filters
				}
			}
		});
		...
		sparrow.accounts.dimensions.setup_dimension_filters(frm, frm.doctype);
		...
		if(!check_should_not_attach_bom_items(frm.doc.bom_no)) {
			sparrow.accounts.dimensions.update_dimension(frm, frm.doctype);
		}
	},
	...
	setup_quality_inspection: function(frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 0) {
			frm.add_custom_button(__("Quality Inspection(s)"), () => {
				let transaction_controller = new sparrow.TransactionController({ frm: frm });
		...
		frm.set_query("quality_inspection", "items", function(doc, cdt, cdn) {
			...
			return {
				query:"sparrow.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	refresh: function(frm) {
		...
		if(frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__('Alternate Item'), () => {
					sparrow.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
			...
			if (frm.doc.add_to_transit && frm.doc.purpose=='Material Transfer' && frm.doc.per_transferred < 100) {
				frm.add_custom_button(__('End Transit'), function() {
					frappe.model.open_mapped_doc({
						method: "sparrow.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm
					})
				});
			}
		...
		if (frm.doc.docstatus===0) {
			frm.add_custom_button(__('Purchase Invoice'), function() {
				sparrow.utils.map_current_doc({
					method: "sparrow.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(__('Material Request'), function() {
				...
				const d = sparrow.utils.map_current_doc({
					method: "sparrow.stock.doctype.material_request.material_request.make_stock_entry",
		...
		if (frm.doc.docstatus===0 && frm.doc.purpose == "Material Issue") {
			frm.add_custom_button(__('Expired Batches'), function() {
				frappe.call({
					method: "sparrow.stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
					callback: function(r) {
						if (!r.exc && r.message) {
							frm.set_value("items", []);
	...
	validate_purpose_consumption: function(frm) {
		frappe.call({
			method: "sparrow.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
		}).then(r => {
			if (cint(r.message) == 0
	...
	set_serial_no: function(frm, cdt, cdn, callback) {
		...
		frappe.call({
			method: "sparrow.stock.get_item_details.get_serial_no",
			args: {"args": args},
			callback: function(r) {
				if (!r.exe && r.message){
	...
	make_retention_stock_entry: function(frm) {
		frappe.call({
			method: "sparrow.stock.doctype.stock_entry.stock_entry.move_sample_to_retention_warehouse",
			args:{
	...
	toggle_display_account_head: function(frm) {
		var enabled = sparrow.is_perpetual_inventory_enabled(frm.doc.company);
	...
	set_basic_rate: function(frm, cdt, cdn) {
		...
		if (item.item_code || item.serial_no) {
			frappe.call({
				method: "sparrow.stock.utils.get_incoming_rate",
				args: {
					args: args
				},
				callback: function(r) {
	...
	get_warehouse_details: function(frm, cdt, cdn) {
		...
		if(!child.bom_no) {
			frappe.call({
				method: "sparrow.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					"args": {
	...
	get_items_from_bom: function(frm) {
		...
		d.get_input("fetch").on("click", function() {
			...
			frappe.call({
				method: "sparrow.manufacturing.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function(r) {
					if (!r.message) {
						frappe.throw(__("BOM does not contain any stock item"));
					} else {
						sparrow.utils.remove_empty_first_row(frm, "items");
	...
	source_warehouse_address: function(frm) {
		sparrow.utils.get_address_display(frm, 'source_warehouse_address', 'source_address_display', false);
	},

	target_warehouse_address: function(frm) {
		sparrow.utils.get_address_display(frm, 'target_warehouse_address', 'target_address_display', false);
	},
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) sparrow.apply_putaway_rule(frm, frm.doc.purpose);
	},
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			sparrow.utils.map_current_doc({
				method: 'sparrow.stock.doctype.stock_entry.stock_entry.get_items_from_subcontract_order',
...
frappe.ui.form.on('Stock Entry Detail', {
	...
	uom: function(doc, cdt, cdn) {
		...
		if(d.uom && d.item_code){
			return frappe.call({
				method: "sparrow.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty
				...
				callback: function(r) {
					if(r.message) {
						...
						if (no_batch_serial_number_value && !frappe.flags.hide_serial_batch_dialog) {
							sparrow.stock.select_batch_and_serial_no(frm, d);
						}
					}
				}
			});
	...
	expense_account: function(frm, cdt, cdn) {
		sparrow.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},
	cost_center: function(frm, cdt, cdn) {
		sparrow.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
...
var validate_sample_quantity = function(frm, cdt, cdn) {
	...
	if (d.sample_quantity && frm.doc.purpose == "Material Receipt") {
		frappe.call({
			method: 'sparrow.stock.doctype.stock_entry.stock_entry.validate_sample_quantity',
			args: {
				batch_no: d.batch_no,
				item_code: d.item_code,
...
sparrow.stock.StockEntry = class StockEntry extends sparrow.stock.StockController {
	setup() {
		var me = this;
		...
		this.frm.fields_dict.items.grid.get_field('item_code').get_query = function() {
			return sparrow.queries.item({is_stock_item: 1});
		};
		...
		if(me.frm.doc.company && sparrow.is_perpetual_inventory_enabled(me.frm.doc.company)) {
			this.frm.add_fetch("company", "stock_adjustment_account", "expense_account");
		}

		this.frm.fields_dict.items.grid.get_field('expense_account').get_query = function() {
			if (sparrow.is_perpetual_inventory_enabled(me.frm.doc.company)) {
				return {
					filters: {
						"company": me.frm.doc.company,
						"is_group": 0
		...
		this.frm.set_query("supplier_address", sparrow.queries.address_query)
	}
	...
	refresh() {
		var me = this;
		sparrow.toggle_naming_series();
		this.toggle_related_fields(this.frm.doc);
		this.toggle_enable_bom();
		this.show_stock_ledger();
		if (this.frm.doc.docstatus===1 && sparrow.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			this.show_general_ledger();
		}
		sparrow.hide_company();
		sparrow.utils.add_item(this.frm);
	}

	scan_barcode() {
		const barcode_scanner = new sparrow.utils.BarcodeScanner({frm:this.frm});
	...
	set_default_account(callback) {
		...
		if(this.frm.doc.company && sparrow.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			return this.frm.call({
				method: "sparrow.accounts.utils.get_company_default",
				args: {
			...
			return this.frm.call({
				...
				callback: function(r) {
					...
					if(me.frm.doc.bom_no) {
						...
						sparrow.accounts.dimensions.update_dimension(me.frm, me.frm.doctype);
					}
				}
			});
		}
	...
	work_order() {
		...
		return frappe.call({
			method: "sparrow.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	set_warehouse_in_children(child_table, warehouse_field, warehouse) {
		let transaction_controller = new sparrow.TransactionController();
		transaction_controller.autofill_warehouse(child_table, warehouse_field, warehouse);
	}

	items_on_form_rendered(doc, grid_row) {
		sparrow.setup_serial_or_batch_no();
	}
	...
	supplier(doc) {
		sparrow.utils.get_party_details(this.frm, null, null, null);
	}
};

sparrow.stock.select_batch_and_serial_no = (frm, item) => {
	...
	frappe.require("assets/sparrow/js/utils/serial_no_batch_selector.js", function() {
		if (frm.batch_selector?.dialog?.display) return;
		frm.batch_selector = new sparrow.SerialNoBatchSelector({
			frm: frm,
			item: item,
...
function attach_bom_items(bom_no) {
	...
	frappe.db.get_doc("BOM",bom_no).then(bom => {
		...
		sparrow.stock.bom = {name, items:{}}
		items.forEach(item => {
			sparrow.stock.bom.items[item.item_code] = item;
		});
	});
}
...
function check_should_not_attach_bom_items(bom_no) {
  return (
	bom_no === undefined ||
	(sparrow.stock.bom && sparrow.stock.bom.name === bom_no)
  );
}

extend_cscript(cur_frm.cscript, new sparrow.stock.StockEntry({frm: cur_frm}));
