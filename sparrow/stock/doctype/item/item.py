...
from sparrow.setup.doctype.item_group.item_group import invalidate_cache_for
from sparrow.stock.doctype.item_default.item_default import ItemDefault
...
class Item(Document):
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": sparrow.get_default_currency(),
	...
	def recalculate_bin_qty(self, new_name):
		from sparrow.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"sparrow.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
...
def invalidate_item_variants_cache_for_website(doc):
	...
	from sparrow.e_commerce.variant_selector.item_variants_cache import ItemVariantsCacheManager
...
def get_asset_naming_series():
	from sparrow.assets.doctype.asset.asset import get_asset_naming_series
