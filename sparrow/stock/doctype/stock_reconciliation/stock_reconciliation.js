...
frappe.ui.form.on("Stock Reconciliation", {
	...
	scan_barcode: function(frm) {
		const barcode_scanner = new sparrow.utils.BarcodeScanner({frm:frm});
	...
	get_items: function(frm) {
		...
		frappe.prompt(fields, function(data) {
			frappe.call({
				method: "sparrow.stock.doctype.stock_reconciliation.stock_reconciliation.get_items",
				args: {
	...
	set_valuation_rate_and_qty: function(frm, cdt, cdn) {
		...
		if(d.item_code && d.warehouse) {
			frappe.call({
				method: "sparrow.stock.doctype.stock_reconciliation.stock_reconciliation.get_stock_balance_for",
				args: {
	...
	toggle_display_account_head: function(frm) {
		frm.toggle_display(['expense_account', 'cost_center'],
			sparrow.is_perpetual_inventory_enabled(frm.doc.company));
	},
	purpose: function(frm) {
		frm.trigger("set_expense_account");
	},
	set_expense_account: function(frm) {
		if (frm.doc.company && sparrow.is_perpetual_inventory_enabled(frm.doc.company)) {
			return frm.call({
				method: "sparrow.stock.doctype.stock_reconciliation.stock_reconciliation.get_difference_account",
				args: {
...
sparrow.stock.StockReconciliation = class StockReconciliation extends sparrow.stock.StockController {
	setup() {
		var me = this;
	...
	refresh() {
		if(this.frm.doc.docstatus > 0) {
			...
			if (sparrow.is_perpetual_inventory_enabled(this.frm.doc.company)) {
...
cur_frm.cscript = new sparrow.stock.StockReconciliation({frm: cur_frm});
