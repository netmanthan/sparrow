...
frappe.provide("sparrow");
frappe.provide("sparrow.utils");
...
$.extend(sparrow, {
	...
	hide_company: function() {
		if(cur_frm.fields_dict.company) {
			...
			} else if(sparrow.last_selected_company) {
				if(!cur_frm.doc.company) cur_frm.set_value("company", sparrow.last_selected_company);
			}
		}
	},
...
$.extend(sparrow.utils, {
	set_party_dashboard_indicators: function(frm) {
		if(frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			var company_wise_info = frm.doc.__onload.dashboard_info;
			if(company_wise_info.length > 1) {
				company_wise_info.forEach(function(info) {
					sparrow.utils.add_indicator_for_multicompany(frm, info);
				});
	...
	get_terms: function(tc_name, doc, callback) {
		if(tc_name) {
			return frappe.call({
				method: 'sparrow.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions',
				args: {
	...
	make_bank_account: function(doctype, docname) {
		frappe.call({
			method: "sparrow.accounts.doctype.bank_account.bank_account.make_bank_account",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	add_dimensions: function(report_name, index) {
		...
		frappe.call({
			method: "sparrow.accounts.doctype.accounting_dimension.accounting_dimension.get_dimensions",
			callback: function(r) {
	...
	add_inventory_dimensions: function(report_name, index) {
		...
		frappe.call({
			method: "sparrow.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function(r) {
	...
	make_pricing_rule: function(doctype, docname) {
		frappe.call({
			method: "sparrow.accounts.doctype.pricing_rule.pricing_rule.make_pricing_rule",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	**/
	...
	get_fiscal_year: function(date) {
		...
		let fiscal_year = '';
		frappe.call({
			method: "sparrow.accounts.utils.get_fiscal_year",
			args: {
				date: date
			},
			async: false,
...
sparrow.utils.select_alternate_items = function(opts) {
	const frm = opts.frm;
		...
		fields: [
			{fieldtype:'Section Break', label: __('Items')},
			{
				...
				fields: [{
					...
					get_query: (e) => {
						return {
							query: "sparrow.stock.doctype.item_alternative.item_alternative.get_alternative_items",
							filters: {
...
sparrow.utils.update_child_items = function(opts) {
	const frm = opts.frm;
	...
	new frappe.ui.Dialog({
		...
		primary_action: function() {
			...
			frappe.call({
				method: 'sparrow.controllers.accounts_controller.update_child_qty_rate',
				freeze: true,
				args: {
...
sparrow.utils.map_current_doc = function(opts) {
	function _map() {
	...
	$.each(frappe.boot.service_level_agreement_doctypes, function(_i, d) {
		frappe.ui.form.on(d, {
			onload: function(frm) {
				...
				frappe.call({
					method: 'sparrow.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters',
					args: {
