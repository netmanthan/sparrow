...
{% include 'sparrow/accounts/doctype/purchase_taxes_and_charges_template/purchase_taxes_and_charges_template.js' %}

cur_frm.email_field = "contact_email";

sparrow.buying.BuyingController = class BuyingController extends sparrow.TransactionController {
	setup() {
		super.setup();
	}
		...
		if(this.frm.get_field('shipping_address')) {
			this.frm.set_query("shipping_address", function() {
				if(me.frm.doc.customer) {
					...
					return sparrow.queries.company_address_query(me.frm.doc)
			});
		}
		/* eslint-enable */
	}
	...
	setup_queries(doc, cdt, cdn) {
		...
		me.frm.set_query('supplier', sparrow.queries.supplier);
		me.frm.set_query('contact_person', sparrow.queries.contact_query);
		me.frm.set_query('supplier_address', sparrow.queries.address_query);

		me.frm.set_query('billing_address', sparrow.queries.company_address_query);
		sparrow.accounts.dimensions.setup_dimension_filters(me.frm, me.frm.doctype);

		if(this.frm.fields_dict.supplier) {
			this.frm.set_query("supplier", function() {
				return{	query: "sparrow.controllers.queries.supplier_query" }});
		}
		...
		this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
			...
			return {
				query: "sparrow.controllers.queries.item_manufacturer_query",
	...
	supplier() {
		var me = this;
		sparrow.utils.get_party_details(this.frm, null, null, function(){
			me.apply_price_list();
		});
	}

	supplier_address() {
		sparrow.utils.get_address_display(this.frm);
		sparrow.utils.set_taxes_from_address(this.frm, "supplier_address", "supplier_address", "supplier_address");
	}
	...
	warehouse(doc, cdt, cdn) {
		...
		if(item.item_code && item.warehouse) {
			return this.frm.call({
				method: "sparrow.stock.get_item_details.get_bin_details",
				child: item,
				args: {
	...
	set_from_product_bundle() {
		...
		this.frm.add_custom_button(__("Product Bundle"), function() {
			sparrow.buying.get_items_from_product_bundle(me.frm);
		}, __("Get Items From"));
	}

	shipping_address(){
		var me = this;
		sparrow.utils.get_address_display(this.frm, "shipping_address",
			"shipping_address_display", true);
	}

	billing_address() {
		sparrow.utils.get_address_display(this.frm, "billing_address",
	...
	manufacturer(doc, cdt, cdn) {
		...
		if(row.manufacturer) {
			frappe.call({
				method: "sparrow.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
				args: {
...
sparrow.buying.link_to_mrs = function(frm) {
	frappe.call({
...
sparrow.buying.get_default_bom = function(frm) {
	$.each(frm.doc["items"] || [], function(i, d) {
		if (d.item_code && d.bom === "") {
			return frappe.call({
				type: "GET",
				method: "sparrow.stock.get_item_details.get_default_bom",
				args: {
...
sparrow.buying.get_items_from_product_bundle = function(frm) {
	var dialog = new frappe.ui.Dialog({
		...
		primary_action(args){
			...
			return frappe.call({
				type: "GET",
				method: "sparrow.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
