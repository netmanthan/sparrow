...
// get tax rate
frappe.provide("sparrow.taxes");
frappe.provide("sparrow.taxes.flags");
...
frappe.ui.form.on(cur_frm.doctype, {
	setup: function(frm) {
		...
		$(frm.wrapper).on('grid-row-render', function(e, grid_row) {
			if(in_list(['Sales Taxes and Charges', 'Purchase Taxes and Charges'], grid_row.doc.doctype)) {
				sparrow.taxes.set_conditional_mandatory_rate_or_amount(grid_row);
			}
		});
	},
	onload: function(frm) {
		if(frm.get_field("taxes")) {
			frm.set_query("account_head", "taxes", function(doc) {
				...
				return {
					query: "sparrow.controllers.queries.tax_account_query",
					filters: {
	...
	taxes_on_form_rendered: function(frm) {
		sparrow.taxes.set_conditional_mandatory_rate_or_amount(frm.open_grid_row());
	},
...
var get_payment_mode_account = function(frm, mode_of_payment, callback) {
	...
	return  frappe.call({
		method: "sparrow.accounts.doctype.sales_invoice.sales_invoice.get_bank_cash_account",
		args: {
...
cur_frm.cscript.account_head = function(doc, cdt, cdn) {
	...
	if(!d.charge_type && d.account_head){
		...
		frappe.call({
			type:"GET",
			method: "sparrow.controllers.accounts_controller.get_tax_rate",
...
if(!sparrow.taxes.flags[cur_frm.cscript.tax_table]) {
	sparrow.taxes.flags[cur_frm.cscript.tax_table] = true;
	...
	frappe.ui.form.on(cur_frm.cscript.tax_table, "charge_type", function(frm, cdt, cdn) {
		...
		if(open_form) {
			sparrow.taxes.set_conditional_mandatory_rate_or_amount(open_form);
		} else {
			// apply in current row
			sparrow.taxes.set_conditional_mandatory_rate_or_amount(frm.get_field('taxes').grid.get_row(cdn));
		}
	});
...
sparrow.taxes.set_conditional_mandatory_rate_or_amount = function(grid_row) {
	if(grid_row) {
