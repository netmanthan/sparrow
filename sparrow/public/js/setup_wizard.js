...
frappe.setup.on("before_load", function () {
	sparrow.setup.slides_settings.map(frappe.setup.add_slide);
});
...
sparrow.setup.slides_settings = [
	{
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				let fy = sparrow.setup.fiscal_years[country];
		...
		charts_modal: function(slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: 'sparrow.accounts.utils.get_coa',
				args: {
