frappe.provide("sparrow.financial_statements");
...
sparrow.financial_statements = {
	...
	"formatter": function(value, row, column, data, default_formatter) {
		if (data && column.fieldname=="account") {
			...
			if (data.account) {
				column.link_onclick =
					"sparrow.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	onload: function(report) {
		...
		sparrow.financial_statements.filters = get_filters()

		let fiscal_year = sparrow.utils.get_fiscal_year(frappe.datetime.get_today());
...
function get_filters() {
	let filters = [
		...
		},
		{
			...
			"fieldtype": "Link",
			"options": "Fiscal Year",
			"default": sparrow.utils.get_fiscal_year(frappe.datetime.get_today()),
			"reqd": 1,
		...
		// accordingly.
		{
			...
			"label": __("Currency"),
			"fieldtype": "Select",
			"options": sparrow.get_presentation_currency_list()
		},
		{
