...
sparrow.SerialNoBatchSelector = class SerialNoBatchSelector {
	...
	make_dialog() {
		...
		let fields = [
			...
			},
			{
				...
				get_query: function() {
					return {
						query: "sparrow.controllers.queries.warehouse_query",
						filters: [
			...
			},
			{
				...
				click: () => {
					...
					let numbers = frappe.call({
						method: "sparrow.stock.doctype.serial_no.serial_no.auto_fetch_serial_number",
						args: {
							qty: qty,
				...
				fields: [
					{
						...
						change: function () {
							...
							if (me.warehouse_details.name) {
								frappe.call({
									method: 'sparrow.stock.doctype.batch.batch.get_batch_qty',
									args: {
										batch_no,
	...
	get_serial_no_fields() {
		...
		if (me.frm.doc.doctype === 'POS Invoice' && !this.showing_reserved_serial_nos_error) {
			frappe.call({
				method: "sparrow.stock.doctype.serial_no.serial_no.get_pos_reserved_serial_nos",
				args: {
					filters: {
						item_code: me.item_code,
						warehouse: typeof me.warehouse_details.name == "string" ? me.warehouse_details.name : '',
...
function get_pending_qty_fields(me) {
	...
	const { qty_consumed_per_unit } = sparrow.stock.bom.items[item_code];
...
function check_can_calculate_pending_qty(me) {
	...
	const docChecks = doc.bom_no
		...
		&& sparrow.stock.bom
		&& sparrow.stock.bom.name === doc.bom_no;
	const itemChecks = !!item
		&& !item.original_item
		&& sparrow.stock.bom && sparrow.stock.bom.items
		&& (item.item_code in sparrow.stock.bom.items);
