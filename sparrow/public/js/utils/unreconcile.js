...
sparrow.accounts.unreconcile_payments = {
	add_unreconcile_btn(frm) {
		if (frm.doc.docstatus == 1) {
			...
			frappe.call({
				"method": "sparrow.accounts.doctype.unreconcile_payments.unreconcile_payments.doc_has_references",
				"args": {
				...
				callback: function(r) {
					if (r.message) {
						frm.add_custom_button(__("UnReconcile"), function() {
							sparrow.accounts.unreconcile_payments.build_unreconcile_dialog(frm);
						}, __('Actions'));
					}
				}
			});
	...
	build_unreconcile_dialog(frm) {
		if (['Sales Invoice', 'Purchase Invoice', 'Payment Entry', 'Journal Entry'].includes(frm.doc.doctype)) {
			...
			// get linked payments
			frappe.call({
				"method": "sparrow.accounts.doctype.unreconcile_payments.unreconcile_payments.get_linked_payments_for_doc",
				"args": {
						...
						let d = new frappe.ui.Dialog({
							...
							primary_action(values) {
								...
								if (selected_allocations.length > 0) {
									let selection_map = sparrow.accounts.unreconcile_payments.build_selection_map(frm, selected_allocations);
									sparrow.accounts.unreconcile_payments.create_unreconcile_docs(selection_map);
									d.hide();
	...
	create_unreconcile_docs(selection_map) {
		frappe.call({
			"method": "sparrow.accounts.doctype.unreconcile_payments.unreconcile_payments.create_unreconcile_doc_for_selection",
			"args": {
